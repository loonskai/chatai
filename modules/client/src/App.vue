<template>
  <section class="relative flex bg-slate-100 m-auto w-120 px-10 pb-20 max-w-screen-sm h-screen">
    <ul class="flex flex-col p-4 pb-10 overflow-y-scroll w-full h-full items-start">
      ...
      <li v-for="message, idx in messages" :key="message.date" class="mb-4 bg-white rounded-md outline outline-1 outline-slate-100 py-2 px-5 shadow-md w-1/2" :class="{'self-start': message.author === 'me', 'self-end': message.author === 'ai'}">
        <div>
          ...
          <div>
            <span :class="{'cursor': loading && message.author === 'ai' && idx === messages.length - 1}">{{message.text}}</span>
          </div>
        </div>
      </li>
    </ul>
    <div class="fixed bottom-4 flex">
      <input
        ...
        @input="update"
        @keyup.enter="submit(requestMessage)"
      />
      <button 
